...
{
	global $conn;
	$sql2 = "select count(*) as allnum from xuanxiu"; //可以显示出总页数
...
function news($pageNum = 1, $pageSize)
{
    ...
    $rs = "select * from xuanxiu limit " . (($pageNum - 1) * $pageSize) . "," . $pageSize;
...
<?php
    foreach($array as $key=>$values){
        ...
        echo "<td><a class='sui-btn btn-small btn-warning' href='grade-update.php?sid={$values['学号']}'>修改</a>&nbsp;&nbsp;
<a class='sui-btn btn-small btn-danger' href='grade-del.php?sid={$values['学号']}'>删除</a></td>";
        echo "</tr>";
    }
?>
			</table>
